...
def train(args):
    ...
    dataset = load_data(args.train_path)
    label = dataset['label'].values

    kfold = model_selection(args)
    for K ,(train_index, dev_index) in enumerate(kfold.split(dataset, label)):
      ...
      training_args = TrainingArguments(
        output_dir=args.save_dir + f'/{K}',          # output directory
      ...
      # train model
      trainer.train()
      path = os.path.join("./best_model", args.wandb_name + f'{K}')
